...
static int gpio_pwm_enable(struct pwm_chip *chip, struct pwm_device *pwm)
{
	...
	if (gpio_data->run) {
		pr_err("gpio pwm %d already running.\n", pwm->pwm);
		return -EBUSY;
	}
...
static void pwm_init_work(struct work_struct *work)
{
	int i;
	struct gpio_pwm_chip *gpio_chip = container_of(work, struct gpio_pwm_chip, init_work);

	pr_info("%s:%d\n", __func__, __LINE__);
	for(i=0; i < gpio_chip->chip.npwm; i++) {
		struct pwm_device *pwm = &gpio_chip->chip.pwms[i];
		gpio_pwm_config(&gpio_chip->chip, pwm, 500000, 1000000 * i);
		gpio_pwm_enable(&gpio_chip->chip, pwm);
	}
}

static int gpio_pwm_remove(struct platform_device *);
static int gpio_pwm_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "num pwm gpio: %d\n", npwm);
	...
	for (i = 0; i < npwm; i++) {
		...
		if (IS_ERR(gpiod)) {
			...
			ret = PTR_ERR(gpiod);
			dev_err(&pdev->dev, "failed to get gpio, ret: %d\n", ret);

			goto __err_release_pwm;
		}
	...
	if (!hrtimer) {
		dev_warn(&pdev->dev, "unable to use High-Resolution timer,\
			%s is restricted to low resolution.", DRV_NAME);
	}

	platform_set_drvdata(pdev, gpio_chip);

	INIT_WORK(&gpio_chip->init_work, pwm_init_work);

	// schedule_work(&gpio_chip->init_work);
...
static int gpio_pwm_remove(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "%d gpio pwms removed\n", gpio_chip->chip.npwm);
	for (i = 0; i < gpio_chip->chip.npwm; i++) {
		struct gpio_pwm_data *gpio_data;
