...
param CFG_ELEMENTS[] =
{
    ...
    */
	...
	{"MaxStaNum", NULL, NULL, hooker,  "8;8;8;8"},
...
void parse_uci(char * arg)
{
    ...
    uci_foreach_element(&uci_wireless->sections, e)
    {
        ...
        if(0 == strcmp(s->type, "wifi-device"))
        {
            ...
            for( i = 0; i<sizeof(CFG_ELEMENTS)/sizeof(CFG_ELEMENTS[0]); i++)
            {
				if(CFG_ELEMENTS[i].dat_key && !strncmp(CFG_ELEMENTS[i].dat_key, "ApCli", 5))
				{
					if(wifi_cfg[cur_dev].params[i].defvalue)
						wifi_cfg[cur_dev].params[i].value = strdup(wifi_cfg[cur_dev].params[i].defvalue);
					else
						wifi_cfg[cur_dev].params[i].value = strdup("");
					printf("%s:%s\n", wifi_cfg[cur_dev].params[i].dat_key, wifi_cfg[cur_dev].params[i].value);
					continue;
				}
    ...
    uci_foreach_element(&uci_wireless->sections, e)
    {
        ...
        {
            value = NULL;
			value = uci_lookup_option_string(uci_ctx, s, "disabled");
			...
			value = uci_lookup_option_string(uci_ctx, s, "mode");
			if(value && 0 == strcmp(value, "sta"))
			{	
				printf("----------------sta mode-------------\n");
				...
				for( i = 0; i<sizeof(CFG_ELEMENTS)/sizeof(CFG_ELEMENTS[0]); i++)
	            {
					if (CFG_ELEMENTS[i].dat_key)
					{
						if (!strcmp(CFG_ELEMENTS[i].dat_key, "ApCliSsid") || !strcmp(CFG_ELEMENTS[i].dat_key, "ApCliBssid") \
							|| !strcmp(CFG_ELEMENTS[i].dat_key, "ApCliWPAPSK"))
		                {
		                    value = NULL;
		                    value = uci_lookup_option_string(uci_ctx, s, CFG_ELEMENTS[i].uci_key);
		                    if(value)
						...
						if (!strcmp(CFG_ELEMENTS[i].dat_key, "ApCliEnable"))
						{
						...
						else if (!strcmp(CFG_ELEMENTS[i].dat_key, "ApCliEncrypType"))
						{
				            ...
				            if (0 == strcasecmp(authmode, "none"))
								wifi_cfg[cur_dev].params[i].value = strdup("NONE");
				            else if (0 == strcasecmp(authmode, "wep-open")
				                     || 0 == strcasecmp(authmode, "wep-shared")
				                     || 0 == strcasecmp(authmode, "8021x"))
								wifi_cfg[cur_dev].params[i].value = strdup("WEP");
				            else if (0 == strcasecmp(cipher, "ccmp"))
								wifi_cfg[cur_dev].params[i].value = strdup("AES");
				            else if (0 == strcasecmp(cipher, "tkip"))
								wifi_cfg[cur_dev].params[i].value = strdup("TKIP");
						...
						else if (!strcmp(CFG_ELEMENTS[i].dat_key, "ApCliAuthMode"))
						{

				            if (!authmode) continue;

				            if (0 == strcasecmp(authmode, "none"))
								wifi_cfg[cur_dev].params[i].value = strdup("OPEN");
				            else if (0 == strcasecmp(authmode, "wep-open"))
								wifi_cfg[cur_dev].params[i].value = strdup("OPEN");
				            else if (0 == strcasecmp(authmode, "8021x"))
								wifi_cfg[cur_dev].params[i].value = strdup("OPEN");
				            else if (0 == strcasecmp(authmode, "wep-shared"))
								wifi_cfg[cur_dev].params[i].value = strdup("SHARED");
				            else if (0 == strcasecmp(authmode, "wep-auto"))
								wifi_cfg[cur_dev].params[i].value = strdup("WEPAUTO");
				            else if (0 == strcasecmp(authmode, "psk"))
								wifi_cfg[cur_dev].params[i].value = strdup("WPAPSK");
				            else if (0 == strcasecmp(authmode, "psk2"))
								wifi_cfg[cur_dev].params[i].value = strdup("WPA2PSK");
				            else if (0 == strcasecmp(authmode, "psk-mixed")
				                     || 0 == strcasecmp(authmode, "psk+psk2"))
								wifi_cfg[cur_dev].params[i].value = strdup("WPAPSKWPA2PSK");
				            else if (0 == strcasecmp(authmode, "wpa"))
								wifi_cfg[cur_dev].params[i].value = strdup("WPA");
				            else if (0 == strcasecmp(authmode, "wpa2"))
								wifi_cfg[cur_dev].params[i].value = strdup("WPA2");
				            else if (0 == strcasecmp(authmode, "wpa-mixed")
				                     || 0 == strcasecmp(authmode, "wpa+wpa2"))
								wifi_cfg[cur_dev].params[i].value = strdup("WPA1WPA2");
				            else
								wifi_cfg[cur_dev].params[i].value = strdup("OPEN");
							printf("%s(),    %s=%s\n", __FUNCTION__, CFG_ELEMENTS[i].dat_key, wifi_cfg[cur_dev].params[i].value);
						}
...
void hooker(FILE * fp, param * p, const char * devname)
{
	...
	else if(0 == strcmp(p->dat_key, "MaxStaNum"))
    {
        for(i=0; i<wifi_cfg[N].vifnum; i++)
        {
            if (i>0) FPRINT(fp, p, ";");
            if (!wifi_cfg[N].vifs[i].maxstanum.value)
                FPRINT(fp, p, "0");
            else
